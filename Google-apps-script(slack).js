...
function fetchUnreadDirectMessages(slackToken, gmailUser, mention) {
  ...
  dms.forEach(function(dm) {
    ...
    var messages = messageData.messages;
     if (messages[0] != null) {
        if (messages[0].ts > lastTimestamp && messages[0].user != mention) {

          const userId = messages[0].user;
            ...
            if (userData.ok) {
                ...
                sendEmail(gmailUser, 'Unread Chart in Slack DM from ' + userName, messages[0].text);
            }else {
              Logger.log(`Error fetching user info for ${userId}: ` + userData.error);
            }
        }
    } else {
    ...
    var messages = messageData.messages.filter(message => message.text.includes(`@${mention}`));
     ...
     if (messages[0] != null) {
          if (messages[0].ts > lastTimestamp &&  messages[0].user != mention) {
            ...
            if (userData.ok) {
                userName = userData.user.real_name || userData.user.name;
                sendEmail(gmailUser, 'Unread Chart in Slack Channel from ' + userName, messages[0].text);
            }else {
              Logger.log(`Error fetching user info for ${userId}: ` + userData.error);
            }
